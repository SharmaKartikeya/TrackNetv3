...
class GenericDataset(Dataset):
    def __init__(self, config):
        ...
        self.base_path = Path(config.dataset)
        self.csvs_dir = self.base_path / config.csvs_dir
        self.sequence_starters = {}
        self.sequence_length = config.sequence_length
        self.one_output_frame = config.one_output_frame
    ...
    def generate_heatmap_2(self, center_x, center_y, width, height):
        ...
        width = int(self.image_size[1]*width)
        height = int(self.image_size[0]*height)

        # image = np.exp(-4*np.log(2) * ((x-x0)**2/width**2 + (y-y0)**2/height**2))
        assert self
        image = np.zeros((self.image_size[0], self.image_size[1]))

        if x0 == 0 and y0 == 0:
            return image
        center_coordinates = (int(x0), int(y0))
        axesLength = (width // 2, height // 2)
        angle = 0
        startAngle = 0
        endAngle = 360
        color = 255
    ...
    def from_dir(config):
        ...
        """
        base_path = Path(config.dataset)
        if (base_path / config.images_dir).is_dir():
            return ImagesDataset(config)
        elif (base_path / config.videos_dir).is_dir():
            return VideosDataset(config)
        else:
            raise Exception(f"No '{config.images_dir}' or '{config.videos_dir}' folder found in dataset.")
...
class ImagesDataset(GenericDataset):
    def __init__(self, config):
        ...
        self.images_folder = self.base_path / config.images_dir
...
class VideosDataset(GenericDataset):
    def __init__(self, config):
        ...
        super().__init__(config)
        self.videos_folder = self.base_path / config.videos_dir
    ...
    def to_images_dataset(self, images_folder="images"):
        ...
        return ImagesDataset(self.config)
...
if __name__ == "__main__":
    ...
    config.dataset = "./example_datasets/video_dataset/"
    config.images_dir = "images"
    config.videos_dir = "videos"
    config.csvs_dir = "csvs"
    config.sequence_length = 3
    config.one_output_frame = False
    config.grayscale = False
    config.image_size = (1024, 512)
    config.include_dups = True

    dataset = GenericDataset.from_dir(config)
